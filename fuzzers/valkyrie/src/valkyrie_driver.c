...
static void maybe_initialize_extra_stats() {
  ...
  if (!extra_stats_filename) return;
  ...
  int crash_signals[] = {SIGSEGV, SIGBUS, SIGABRT, SIGILL,
                         SIGFPE,  SIGINT, SIGTERM};
...
static void maybe_duplicate_stderr() {
  ...
  if (!stderr_duplicate_stream) {
    fprintf(stderr,
            "Failed to duplicate stderr to "
            "ANGORA_DRIVER_STDERR_DUPLICATE_FILENAME");
    abort();
  }
}
